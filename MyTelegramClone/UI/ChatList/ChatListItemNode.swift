...
// MARK: - Портированная версия ChatListItemNode из оригинального Telegram iOS
// Источник: mytelegram-iOS/submodules/ChatListUI/Sources/Node/ChatListItem.swift
// Изменения: AsyncDisplayKit → UIKit, упрощённые зависимости

// MARK: - Заглушки для оригинальных типов
...
// MARK: - ChatListItemContent (из оригинала, строки 33-252)
...
// MARK: - ChatListItem (из оригинала, строка 454+)
...
// MARK: - ChatListItemNode (ОРИГИНАЛ ИЗ TELEGRAM, адаптация с AsyncDisplayKit на UIKit)
// Источник: строки 993-5357 в ChatListItem.swift

class ChatListItemNode: UITableViewCell {
    
    // MARK: - Constants (из оригинала)
    ...
    // MARK: - Setup (из оригинала, адаптация)
    ...
    // MARK: - Layout (из оригинала, строки 1500+)
    ...
    override func layoutSubviews() {
        ...
        // Container с учётом swipe offset
    ...
    // MARK: - Peer Item Configuration (из оригинала)
    ...
    private func configurePeerItem(
        ...
        // Verified icon (для официальных аккаунтов)
        verifiedIconView.isHidden = true // TODO: добавить проверку
        ...
        // Online dot (TODO: добавить проверку online статуса)
        onlineDotView.isHidden = true
        
        // Typing indicator
        typingIndicatorView.isHidden = true // TODO: WebSocket для typing
    }
    ...
    private func configureGroupItem(
        ...
        // Avatar (группа аватаров)
        configureGroupAvatar(peers: peers)
        
        // Title
        titleLabel.text = "Group Chat" // TODO: название группы
    ...
    private func configureAvatar(for peer: EnginePeer) {
        if let avatarUrl = peer.avatarUrl, let _ = URL(string: avatarUrl) {
            ...
            // Fallback: первая буква + цветной фон
            ...
            // Генерация цвета по peer.id (как в оригинале)
            let colors: [UIColor] = [
    ...
    private func configureGroupAvatar(peers: [EnginePeer]) {
        // Простая версия: показываем первую букву первого участника
        if let firstPeer = peers.first {
            configureAvatar(for: firstPeer)
        }
    }
    
    // MARK: - Date Formatting (из оригинала)
    ...
    // MARK: - Swipe Actions (из оригинала, строки 3000+)
    ...
    @objc private func handlePan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            // Swipe right для reply (left actions)
            if translation.x > 0 {
                revealOffset = min(translation.x, 80)
            }
            // Swipe left для delete/pin (right actions)
...
// MARK: - TypingIndicatorView (из оригинала, анимация трёх точек)
